...
// evicts an entry when that job's live state changes. It ports the Python
// _job_info_cache plus _invalidate_changed_job_info (commit 77e57c3): the modal
// flow consults Get with the job's current state and only reuses a cached entry
// when the state matches; SyncStates evicts entries whose state has drifted (or
// that vanished) after a store refresh.
...
// matching normalize_array_job_id used by the Python cache. A plain "12345" or
// "12345_0" is returned unchanged; "12345_[0-9]" collapses to "12345".
...
// refresh so the modal cache stays consistent with the live job list. Ports
// _invalidate_changed_job_info.
